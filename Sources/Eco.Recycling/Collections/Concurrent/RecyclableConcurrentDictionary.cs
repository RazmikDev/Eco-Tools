...
namespace Eco.Recycling.Collections.Concurrent
{
	/// <summary>
	/// Represents <see cref="ConcurrentDictionary{TKey,TValue}"/> that supports cascade recycling of itself and all children.
	/// </summary>
	/// <typeparam name="TKey">The type of the key.</typeparam>
	/// <typeparam name="TValue">The type of the value.</typeparam>
	public class RecyclableConcurrentDictionary<TKey, TValue> : ConcurrentDictionary<TKey, TValue>, IRecyclable
	{
		#region Fields

		/// <summary>
		/// The  recycle factory that creates and recycles the <see cref="RecyclableConcurrentDictionary{TKey,TValue}"/> objects withing current thread.
		/// </summary>
		public static readonly RecycleFactory<RecyclableConcurrentDictionary<TKey, TValue>> CurrentThreadFactory;

		/// <summary>
		/// The global recycle factory that creates and recycles the <see cref="RecyclableConcurrentDictionary{TKey,TValue}"/> objects.
		/// </summary>
		public static readonly RecycleFactory<RecyclableConcurrentDictionary<TKey, TValue>> Factory;
		...
		/// <summary>
		/// Creates a type instance of the <see cref="RecyclableConcurrentDictionary{TKey,TValue}"/> class.
		/// </summary>
		static RecyclableConcurrentDictionary()
		{
			CurrentThreadFactory = new DefaultRecycleFactory<RecyclableConcurrentDictionary<TKey, TValue>>(StoragePolicy.ThreadStatic);
			Factory = new DefaultRecycleFactory<RecyclableConcurrentDictionary<TKey, TValue>>(StoragePolicy.Concurrent);
		}

		/// <summary>
		/// Initializes a new instance of the <see cref="RecyclableConcurrentDictionary{TKey, TValue}"/> class.
		/// </summary>
		public RecyclableConcurrentDictionary()
		{
			RecycleItems = true;
		}

		/// <summary>
		/// Initializes a new instance of the <see cref="RecyclableConcurrentDictionary{TKey, TValue}"/> class with <paramref name="comparer"/>.
		/// </summary>
		...
		public RecyclableConcurrentDictionary(IEqualityComparer<TKey> comparer)
			: base(comparer)
		{
			RecycleItems = true;
		}
